...
`/api/forms/membership` only forwards submissions into the intranet (`lib/intranetSignup.ts`) when `INTRANET_SIGNUP_URL` is set; otherwise it falls back to the existing notify/email path only.
