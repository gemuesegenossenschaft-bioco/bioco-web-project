<?php
/**
 * Rich text block render template.
...
$anchor = !empty($block['anchor']) ? $block['anchor'] : 'rich-text';

$class_name = 'cms-section cms-rich-text';
if (!empty($block['className'])) {
    $class_name .= ' ' . $block['className'];
}

// Mirror SectionHeader: suppress the standalone title when the WYSIWYG
...
<section id="<?php echo esc_attr($anchor); ?>" class="<?php echo esc_attr($class_name); ?>" data-style-variant="<?php echo esc_attr($style_variant); ?>">
    ...
    <?php if (!empty($buttons)) : ?>
        <div class="cms-section-actions">
            ...
            ?>
                <a href="<?php echo esc_url($button_href); ?>" class="btn btn-<?php echo esc_attr($button_variant); ?> btn-organic"><?php echo esc_html($button_text); ?></a>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
</section>
