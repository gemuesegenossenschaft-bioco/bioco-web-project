...
function bioco_forms_message_schema() {
    ...
    return [
        ...
        'membership-form' => ['fallback_error', 'captcha_error', 'transport_error'],
        ...
        'shared' => ['missing_token', 'invalid_token', 'required_field', 'privacy', 'email', 'membership', 'captcha', 'generic', 'missing_fields', 'mail_failed', 'newsletter_intro', 'newsletter_expiry', 'newsletter_ignore', 'newsletter_subject', 'newsletter_greeting'],
    ];
}
...
function bioco_forms_messages_page() {
    ...
    foreach (bioco_forms_message_schema() as $group => $keys) {
        ...
        foreach ($keys as $key) {
            $id = $group . '-' . $key;
            echo '<tr><th><label for="' . esc_attr($id) . '">' . esc_html($labels[$key]) . '</label></th><td>';
