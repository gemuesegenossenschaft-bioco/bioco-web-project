                ...
                foreach (['section_eyebrow' => 'section_eyebrow', 'section_text' => 'section_text'] as $seedKey => $label) {
                    if (!empty($section[$seedKey])) {
                        ...
                        $headingOnly = $seedKey === 'section_text'
                            ...
                            && trim(strip_tags(html_entity_decode(strip_tags($heading[1]), ENT_QUOTES | ENT_HTML5, 'UTF-8'))) === trim((string) ($section['section_title'] ?? ''));
                        if ($headingOnly) {
    ...
    foreach ($sectionGroup as $section) {
        ...
        if (array_key_exists('section_text', $section)) {
            ...
            if (preg_match('/^\s*<h[1-6][^>]*>(.*?)<\/h[1-6]>\s*/is', $sourceText, $heading)) {
                $headingText = trim(strip_tags(html_entity_decode(strip_tags($heading[1]), ENT_QUOTES | ENT_HTML5, 'UTF-8')));
