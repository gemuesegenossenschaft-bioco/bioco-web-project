...
Font loading note (not a `--wp--*` var, so outside the conformance check, but worth stating): the
literal `@font-face` for DM Sans (`theme.json` -> `assets/fonts/dmsans-variable.woff2`) is generated
by WP core's global-styles output the same way the custom properties are, and has the same "only
exists under the block theme" problem. It is **out of scope for this issue** (#101 is about block/CSS
variable portability, not font loading) but must be solved before Divi go-live — tracked as a
follow-up: either ship the woff2 + an explicit `@font-face` in `bioco-tokens.css`, or let the Divi
child theme's own typography settings supply a font. Noted here so it isn't lost.
...
- `style.css` — theme header only, `Template: Divi`, German description, no design tokens (Divi
  supplies its own theme options UI; `bioco-core` supplies block styling independent of theme):
  ```css
  /*
  Theme Name: bioco Divi Child
  Template: Divi
  Description: Dünnes Kind-Theme für Divi 5. Enthält keine Inhalte, Blöcke oder Formulare — diese kommen aus den mu-plugins bioco-core/bioco-content/bioco-forms. Divi selbst ist lizenzpflichtig, wird über wp-admin > Themes > Hochladen installiert und niemals ins Repo eingecheckt (siehe README.md in diesem Ordner und web/app/themes/Divi in .gitignore).
  Author: bioco
  Version: 1.0.0
  */
  ```
- `functions.php` — enqueues the parent Divi stylesheet, nothing else:
  ```php
  <?php
  if (!defined('ABSPATH')) exit;
  ...
  ```
  (`get_template_directory_uri()` here is correct and intentional — this is the one place in the
  whole restructure where a theme-relative helper is *right*: it targets the **parent Divi theme's**
  stylesheet, which is a theme concern, not a block/content concern; pattern 1/2 in the other doc are
  about the *bioco block theme's* asset paths, not this child theme's own parent enqueue.)
